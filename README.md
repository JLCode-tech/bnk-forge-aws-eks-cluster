...
| [`modules/eks-cluster-install-multus`](./modules/eks-cluster-install-multus) | **Vendored** from `bnk-forge-catalog-shared`. Cloud-agnostic Multus CNI install — applies the upstream daemonset, waits for the `NetworkAttachmentDefinition` CRD + DS rollout. No cloud bits. | Implemented (alpha) — vendored |
| [`modules/eks-cluster-install-tmm-nads`](./modules/eks-cluster-install-tmm-nads) | AWS-specific. Discovers the `f5-bnk-role`-tagged TMM subnets and creates the two NADs (`ens7-ipvlan-l2` external, `ens8-ipvlan-l2` internal) with placeholder addresses derived from the discovered CIDRs. Depends on `install-multus` via the `multus_ready` gate. | Implemented (alpha) |
