...
| [`modules/eks-cluster-install-tmm-nads`](./modules/eks-cluster-install-tmm-nads) | Install **Multus CNI** + the two NetworkAttachmentDefinitions TMM needs (`ens7-ipvlan-l2` external, `ens8-ipvlan-l2` internal). AWS EKS doesn't ship Multus by default; the variant blueprints chain this between `hp-nodes` and `bnk-prereqs` so the NAD prerequisite from the F5 install guide is handled automatically. | Implemented (alpha) |
